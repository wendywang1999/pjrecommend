...
class DPGNN(GeneralRecommender):
    ...
    def predict(self, interaction):
        ...
        scores = I_geek + I_job
        # 创建数据框 df1
        df1 = pd.DataFrame({'求职者ID': user, '招聘者ID': item, '求职者满意度': scores})
        # 对求职者满意度一列进行3sigma异常值检验
        df1['求职者满意度'] = np.where(np.abs(zscore(df1['求职者满意度'])) > 3, 0, df1['求职者满意度'])
        # 保存 df1 为 result3-2.csv 文件
        df1.to_csv('result3-2.csv', index=False)
        # 读取 result3-1.csv 文件作为数据框 df2
        df2 = pd.read_csv('result3-1.csv')
        # 将岗位匹配度的值复制到 df1 中
        df1['岗位匹配度'] = df1.apply(lambda x: df2.loc[(df2['招聘者ID'] == x['招聘者ID']) & (df2['求职者ID'] == x['求职者ID']), '岗位匹配度'].values[0] if ((df2['招聘者ID'] == x['招聘者ID']) & (df2['求职者ID'] == x['求职者ID'])).any() else np.nan, axis=1)
        # 保存合并后的数据框为 result4.csv 文件
